...
*/
fn fileinfo_filename<T>(
    info: &mut CommandInfo<T>,
    attribute: NtfsAttribute,
    ftimes: &mut FileTimestamps,
) where
    T: Read + Seek,
{
    ...
    ftimes.mft_record =
        get_fn_times(file_name.mft_record_modification_time().nt_timestamp() as i64);
}
...
pub fn get_fname(
    path: &Path,
    mut ftimes: FileTimestamps,
) -> Result<(FileTimestamps, Vec<DataRun>)> {
...
fn bytes_to_string(bytes: &Vec<u8>) -> io::Result<String> {
    ...
    let first_bytes_as_string = s.as_str().replace('\u{0}', ".").to_string();
